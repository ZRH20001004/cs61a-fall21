...
def is_bst(t):
    ...
    if t.is_leaf() or len(t.branches) == 1:
        return True
    b = t.branches
    if bst_min(b[0]) <= bst_max(b[0]) and bst_min(b[1]) <= bst_max(b[1]) and bst_max(b[0]) <= bst_min(b[1]):
        ans = True
        for i in t.branches:
            ans = ans and is_bst(i)
        return ans
    else:
        return False
...
def bst_max(t):
    if t.is_leaf():
       ...
       return max(t.label, t.branches[1].label)
    else:
