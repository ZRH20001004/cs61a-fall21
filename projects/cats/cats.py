...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        squences = [[typed_word,valid_words[i]] for i in range(len(valid_words))]
        s = min(squences,key=lambda squence:diff_function(squence[0],squence[1],limit))
        if diff_function(s[0],s[1],limit) > limit:
            return typed_word
        else:
            return s[1]
...
def feline_flips(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    ab = abs(len(goal) - len(start))
    return ab + helper(start,goal,limit - ab + 1)
...
def helper(start, goal, limit):
    ...
    if len(start) == 0 or len(goal) == 0:
        ...
        if start[0] != goal[0]:
            return 1 + helper(start[1:],goal[1:],limit - 1)
        else:
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    p = times_per_player
    times = [[p[i][j + 1]-p[i][j] for j in range(len(words))] for i in range(len(p))]
...
def fastest_words(match):
    ...
    for word_indice in word_indices:
        ...
        for player_indice in player_indices:
            if time(match,player_indice,word_indice) < min:
                min = time(match,player_indice,word_indice)
                player = player_indice
        fastest[player].append(word_at(match,word_indice))
    return fastest
    # END PROBLEM 10
