...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        range = 0
        while not current_place.is_hive and range <= self.max_range:
            if range >= self.min_range and len(current_place.bees) > 0:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.slow_turns:
            ...
            if self.health > 0 and gamestate.time % 2 == 0:
                if self.back_times:
                    if not back.is_hive:
                        self.move_to(back)
                    self.back_times -= 1
            ...
            self.slow_turns -= 1
        elif self.back_times:
            back = self.place.entrance
            if self.blocked():
                self.sting(self.place.ant)
            elif not back.is_hive and self.health > 0:
                self.move_to(back)
            self.back_times -= 1
        else:
            if self.blocked():
    ...
    def scare(self, length):
        ...
        # BEGIN Problem EC
        if self.can_scared:
            self.back_times = length
            self.can_scared = False
        # END Problem EC
