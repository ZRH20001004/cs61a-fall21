...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
            ...
            env.define(signature, lambda_procedure)
            return signature
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    if is_scheme_false(result) or len(expressions) == 1:
        return result
...
def do_or_form(expressions, env):
    ...
    if len(expressions) == 0:
        return False
    result = scheme_eval(expressions.first, env)
    if is_scheme_true(result) or len(expressions) == 1:
        return result
...
def getnames(bindings):
    binding = bindings.first
    if binding is not Pair:
        return Pair(bindings.first, nil)
    return Pair(binding.first, getnames(bindings.rest))

def getvalues(bindings):
    binding = bindings.first
    if binding is not Pair:
        return Pair(bindings.rest.first, nil)
    return Pair(binding.rest.first, getvalues(bindings.rest))
